...
void loop() {
  ...
  if (currentTime - lastTime >= control_interval) {
    ...
    float error = desired_velocity - velocity;
    float control_voltage = kp * error;

    // Add this to current voltage to get output
    float output_voltage = control_voltage;
    output_voltage = constrain(output_voltage, 0.0, max_dac_voltage);

    // Send voltage to DAC
    dac.setVoltage(output_voltage / faktor_skala, false);
